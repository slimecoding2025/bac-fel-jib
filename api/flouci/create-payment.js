...
export default async function handler(req, res) {
  ...
  try {
    const flouciResponse = await fetch(`${FLOUCI_BASE_URL}/generate_payment`, {
      method: "POST",
      headers: {
    ...
    if (!flouciResponse.ok || !paymentUrl || !paymentId) {
      return res.status(400).json({ message: data?.message ?? "Could not generate Flouci payment." });
    }

    return res.status(200).json({ payment_url: paymentUrl, payment_id: paymentId, tracking_id: trackingId });
  } catch {
    return res.status(500).json({ message: "Flouci service unavailable." });
  }
}