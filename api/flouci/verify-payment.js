...
export default async function handler(req, res) {
  ...
  /*
  // Future Flouci verification code

  const authToken = process.env.FLOUCI_AUTH_TOKEN;

  if (!authToken) {
    return res.status(500).json({
      message: "Missing Flouci token."
    });
  }
  ...
  try {
    const flouciResponse = await fetch(
      `${FLOUCI_BASE_URL}/verify_payment/${encodeURIComponent(paymentId)}`,
      {
        method: "GET",
        headers: {
    ...
    return res.status(500).json({
      status: "FAILURE",
      message: "Flouci verification unavailable."
    });
  }
  */
}